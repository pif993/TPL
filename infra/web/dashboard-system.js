(() => {
  ...
  const mcSwitchTab = (tab) => {
    ...
    // Lazy load tab data
    if (tab === 'version') { mcLoadVersion(); otaLoadStatus(); }
  ...
  const otaLoadStatus = async () => {
    try {
      ...
      // Check info
      const info = q('otaCheckInfo');
      if (info) {
        info.textContent = data.last_check
          ? `Ultimo controllo: ${otaFmtDate(data.last_check_iso)} · ${data.check_count || 0} controlli`
          : 'Ultimo controllo: mai';
      }

      // Rate limit
      const rl = q('otaRateLimit');
      if (rl) {
      ...
      // Config fields
      const cfgAuto = q('otaCfgAutoCheck');
      const cfgInterval = q('otaCfgInterval');
      const cfgPre = q('otaCfgPreRelease');
      ...
      otaUpdateSidebarBadge(data.update_available);
  ...
  const otaUpdateSidebarBadge = (hasUpdate) => {
    let badge = document.getElementById('otaSidebarBadge');
    const navModules = document.querySelector('[data-view="modules"]');
    if (!navModules) return;
    if (hasUpdate) {
      if (!badge) {
        badge = document.createElement('span');
        badge.id = 'otaSidebarBadge';
        badge.className = 'ota-sidebar-badge';
        badge.title = 'Aggiornamento disponibile';
        badge.innerHTML = '<i class="bi bi-arrow-up-circle-fill"></i>';
        navModules.style.position = 'relative';
        navModules.appendChild(badge);
      }
      badge.style.display = '';
    } else if (badge) {
  ...
  const mcOtaCheck = async () => {
    ...
    try {
        ...
        if (notif) {
          ...
          notif.innerHTML = `
            <div class="ota-notif-body">
              ...
              <button class="btn btn-primary btn-sm" id="otaShowReleasesBtn" type="button"><i class="bi bi-list-ul"></i> Vedi release</button>
            </div>`;
          q('otaShowReleasesBtn')?.addEventListener('click', otaLoadReleases);
        }
        otaLoadReleases();
      } else {
        const notif = q('otaNotification');
      ...
      const badge2 = q('otaStatusBadge');
      if (badge2) {
        badge2.className = 'ota-status-badge ota-badge-error';
        badge2.innerHTML = `<i class="bi bi-exclamation-triangle-fill"></i> Errore`;
      }
      showMessage(`Controllo OTA fallito: ${error}`, 'danger');
    }
  };

  const otaLoadReleases = async () => {
    try {
      const data = await TPL.jsonFetch('/api/ota/releases');
      _otaReleases = data.releases || [];

      const container = q('otaReleaseList');
      if (!container) return;
      container.className = 'ota-release-list';

      if (!_otaReleases.length) {
        container.innerHTML = '<div class="text-muted small p-2"><i class="bi bi-inbox"></i> Nessuna release trovata</div>';
        return;
      }

      container.innerHTML = `
        <div class="ota-releases-header">
          <h6 class="mb-0"><i class="bi bi-tag-fill"></i> Release disponibili</h6>
          <span class="ota-pill">${_otaReleases.length} release</span>
        </div>
          ...
          ${_otaReleases.map(r => `
            <div class="ota-release-card ${r.is_current ? 'ota-rel-current' : ''} ${r.is_newer ? 'ota-rel-newer' : ''} ${r.is_prepared ? 'ota-rel-prepared' : ''}"
              ...
              <div class="ota-rel-meta">
                <span><i class="bi bi-calendar3"></i> ${otaFmtDate(r.published_at)}</span>
                ${r.author ? `<span><i class="bi bi-person"></i> ${esc(r.author)}</span>` : ''}
              </div>
              ${r.body ? `<div class="ota-rel-body">${esc(r.body).substring(0, 200)}${r.body.length > 200 ? '…' : ''}</div>` : ''}
              <div class="ota-rel-actions">
                ...
                ${r.is_prepared ? `<button class="btn btn-success btn-sm" data-ota-install="${esc(r.tag)}" type="button"><i class="bi bi-play-circle"></i> Installa</button>` : ''}
              </div>
            </div>
          `).join('')}
        </div>`;
      ...
      container.querySelectorAll('[data-ota-detail]').forEach(btn =>
        btn.addEventListener('click', () => otaShowDetail(btn.dataset.otaDetail))
      );
      container.querySelectorAll('[data-ota-prepare]').forEach(btn =>
        btn.addEventListener('click', () => otaPrepare(btn.dataset.otaPrepare))
      );
      container.querySelectorAll('[data-ota-install]').forEach(btn =>
        btn.addEventListener('click', () => otaShowDetail(btn.dataset.otaInstall))
      );
    } catch (error) {
  ...
  const otaShowDetail = async (tag) => {
    _otaSelectedTag = tag;
    const container = q('otaReleaseDetail');
    if (!container) return;

    container.className = 'ota-release-detail';
    ...
    try {
      ...
      let html = `
        <div class="ota-detail-header">
          <button class="btn btn-link btn-sm p-0" id="otaBackToList" type="button"><i class="bi bi-arrow-left"></i> Torna alla lista</button>
        ...
        <div class="ota-detail-section">
          ...
          <div class="ota-release-notes">${detail.body ? otaRenderMarkdown(detail.body) : '<span class="text-muted">Nessuna nota disponibile</span>'}</div>
        </div>

        <!-- Meta -->
        <div class="ota-detail-meta">
          <span><i class="bi bi-calendar3"></i> Pubblicato: ${otaFmtDate(detail.published_at)}</span>
        ...
        <div class="ota-detail-section">
          ...
          ${diff.commits?.length ? `
            <div class="ota-commits-list">
              ...
              `).join('')}
              ${diff.commits.length > 15 ? `<div class="text-muted small">… e altri ${diff.commits.length - 15} commit</div>` : ''}
            </div>` : ''}
      ...
      // Changed files list (if prepared)
        ...
        <div class="ota-detail-section ota-install-guide">
          ...
          <div class="ota-rollback-info mt-3">
            ...
            <div class="ota-step-cmd">
              ...
              <button class="btn btn-link btn-sm ota-copy-btn" data-copy="${esc(guide.rollback.command)}" title="Copia comando" type="button"><i class="bi bi-clipboard"></i></button>
            </div>
          </div>
          ${guide.notes?.length ? `
      ...
      html += `
        <div class="ota-detail-actions">
          ${detail.is_newer && !detail.is_prepared ? `<button class="btn btn-success" id="otaPrepareBtnDetail" type="button"><i class="bi bi-download"></i> Scarica e prepara aggiornamento</button>` : ''}
          ${detail.is_prepared ? `<button class="btn btn-outline-danger btn-sm" id="otaCleanupBtn" type="button"><i class="bi bi-trash"></i> Rimuovi staging</button>` : ''}
          ${detail.is_newer ? `<button class="btn btn-outline-secondary btn-sm" id="otaDismissBtn" type="button"><i class="bi bi-bell-slash"></i> Ignora versione</button>` : ''}
        </div>`;

      container.innerHTML = html;

      // Attach listeners
      q('otaBackToList')?.addEventListener('click', () => {
        container.className = 'ota-release-detail d-none';
        q('otaReleaseList')?.classList.remove('d-none');
      });
      q('otaPrepareBtnDetail')?.addEventListener('click', () => otaPrepare(tag));
      q('otaCleanupBtn')?.addEventListener('click', () => otaCleanupStaging(tag));
      q('otaDismissBtn')?.addEventListener('click', () => otaDismiss(tag));
      ...
      // Hide release list, show detail
      q('otaReleaseList')?.classList.add('d-none');
  ...
  const otaPrepare = async (tag) => {
    const progress = q('otaProgress');
    if (progress) {
    ...
    try {
      ...
      showMessage(`Versione ${tag} preparata con successo. Consulta la guida installazione.`, 'success');

      // Reload detail to show install guide
      setTimeout(() => otaShowDetail(tag), 500);
      await otaLoadStatus();
    } catch (error) {
      if (progress) {
      ...
      showMessage(`Preparazione ${tag} fallita: ${error}`, 'danger');
    }
  };

  const otaCleanupStaging = async (tag) => {
    ...
    try {
      ...
      showMessage(`Staging per ${tag} rimosso`, 'success');
      await otaLoadStatus();
      otaLoadReleases();
      q('otaReleaseDetail').className = 'ota-release-detail d-none';
    } catch (error) {
      showMessage(`Errore rimozione staging: ${error}`, 'danger');
    }
  };

  const otaDismiss = async (tag) => {
    try {
      ...
      showMessage(`Notifica per ${tag} silenziata`, 'info');
      await otaLoadStatus();
    } catch (error) {
      showMessage(`Errore: ${error}`, 'danger');
    }
  };

  const otaSaveConfig = async () => {
    try {
      const cfg = {
        auto_check: q('otaCfgAutoCheck')?.checked ?? true,
        check_interval_minutes: parseInt(q('otaCfgInterval')?.value || '60', 10),
        pre_release: q('otaCfgPreRelease')?.checked ?? false,
      };
      ...
      if (res) { res.innerHTML = '<span class="text-success"><i class="bi bi-check-lg"></i> Salvato</span>'; setTimeout(() => { res.textContent = ''; }, 2000); }
    } catch (error) {
      const res = q('otaCfgResult');
  ...
  const bindEvents = () => {
    ...
    q('otaCfgSaveBtn')?.addEventListener('click', otaSaveConfig);

    // Auto-load OTA status on page load for sidebar badge
    otaLoadStatus();
